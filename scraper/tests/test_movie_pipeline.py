...
class TestMoviePipeline:
    ...
    def test_process_item_returns_cleaned_dict_on_valid_item(self):
        item = {"source_name": "  ABC-123  ", "code": "ABC-123", "source_url": "http://example.com"}
        result = self.pipeline.process_item(item)
        assert result is not None
        assert result["source_name"] == "ABC-123"

    def test_process_item_returns_none_on_invalid_item(self):
        item = {"source_name": "", "code": "", "source_url": ""}
        result = self.pipeline.process_item(item)
        assert result is None

    def test_process_items_returns_list_of_valid_items(self):
        items = [
            {"source_name": "A", "code": "A"},
            {"source_name": "", "code": "", "source_url": ""},
            {"source_name": "B", "code": "B"},
        ]
        cleaned = self.pipeline.process_items(items)
        assert len(cleaned) == 2

    def test_has_chinese_sub_adds_tag(self):
        item = {"source_name": "test", "code": "T-001", "has_chinese_sub": True, "tags": ["4K"]}
        result = self.pipeline.process_item(item)
