...
import {
  fetchStorageTasks,
  fetchStorageTaskStats,
  retryStorageTask,
  cancelStorageTask,
  deleteStorageTask,
  batchRetryStorageTasks,
  batchCancelStorageTasks,
  batchDeleteStorageTasks,
  statusColors,
  statusLabels,
  stepLabels,
  type StorageTask,
  type StorageTaskStats,
} from "./api";
...
export default function StorageTaskList() {
  ...
  const [stats, setStats] = useState<StorageTaskStats>({
    pending: 0,
    waiting_download: 0,
    running: 0,
    waiting_retry: 0,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await fetchStorageTasks(params);
      setTasks(data.items);
      setTotal(data.total);
  ...
  const loadStats = useCallback(async () => {
    try {
      const data = await fetchStorageTaskStats();
      setStats(data);
    } catch {
  ...
  const handleCancel = useCallback(async (taskId: string) => {
    try {
      await cancelStorageTask(taskId);
  ...
  const handleDelete = useCallback(async (taskId: string) => {
    try {
      await deleteStorageTask(taskId);
  ...
  const handleBatchRetry = useCallback(async () => {
    ...
    try {
      const result = await batchRetryStorageTasks(selectedRowKeys as string[]);
  ...
  const handleBatchCancel = useCallback(async () => {
    if (selectedRowKeys.length === 0) return;
    try {
      const result = await batchCancelStorageTasks(selectedRowKeys as string[]);
  ...
  const handleBatchDelete = useCallback(async () => {
    if (selectedRowKeys.length === 0) return;
    try {
      const result = await batchDeleteStorageTasks(selectedRowKeys as string[]);
  ...
  // ── Table columns ──────────────────────────────────────
  const columns: ColumnsType<StorageTask> = useMemo(
    () => [
      {
        title: "任务ID",
        dataIndex: "task_id",
