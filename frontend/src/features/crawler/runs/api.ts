...
export function fetchRuns(params?: {
  ...
  return client.get("/crawler/runs", { params }).then((res) => res.data);
}

export function fetchRun(id: string): Promise<TaskRun> {
  return client.get(`/crawler/runs/${id}`).then((res) => res.data);
}

export function fetchQueueStatus(): Promise<QueueStatus> {
  return client.get("/crawler/runs/queue-status").then((res) => res.data);
}

export function stopRun(id: string): Promise<{ success: boolean; message: string }> {
  return client.post(`/crawler/runs/${id}/stop`).then((res) => res.data);
}

export function deleteRun(id: string): Promise<{ deleted: boolean }> {
  return client.delete(`/crawler/runs/${id}`).then((res) => res.data);
}
...
export function fetchRunDetailTasks(
  ...
  return client.get(`/crawler/runs/${runId}/tasks`, { params }).then((res) => res.data);
}

export function retryCrawl(runId: string, taskId: string): Promise<{ success: boolean; message?: string }> {
  return client.post(`/crawler/runs/${runId}/tasks/${taskId}/retry-crawl`).then((res) => res.data);
}

export function retrySave(runId: string, taskId: string): Promise<{ success: boolean; message?: string }> {
  return client.post(`/crawler/runs/${runId}/tasks/${taskId}/retry-save`).then((res) => res.data);
}
