...
export default function StorageConfig() {
  ...
  const handleSave = async (values: StorageConfigType) => {
    setSaving(true);
    try {
      const payload: Partial<StorageConfigType> = { ...values };
      if (tokenInput) {
      ...
      const fresh = await fetchStorageConfig();
